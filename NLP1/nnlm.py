...
def main():
    ...
    with graph.as_default():
        # 定义训练数据
        input_data = tf.placeholder(tf.int32, [args.batch_size, args.win_size])  # 有n个单词
        targets = tf.placeholder(tf.int64, [args.batch_size, 1])  # 输出为概率

        # 模型参数
        with tf.variable_scope('nnlm' + 'embedding'):
            embeddings = tf.Variable(tf.random_uniform([args.vocab_size, args.word_dim], -1.0, 1.0))  # 单词的个数，特征的未读
            embeddings = tf.nn.l2_normalize(embeddings, 1)  # 使用L2范数正则化
        ...
        def infer_output(input_data):
            ...
            input_data_embedding = tf.nn.embedding_lookup(embeddings,input_data)  # (batch_size, win_size, word_dim)
            input_data_embedding = tf.reshape(input_data_embedding,[-1,args.win_size * args.word_dim])
            hidden = tf.tanh(tf.matmul(input_data_embedding, weight_h) + b1)
            hidden_output = tf.matmul(input_data_embedding, softmax_w) + tf.matmul(hidden, softmax_u) + b2
